# This script will run ALL of the sites in the Alpine protocol
...
# We need to get ALL of the raw data to identify which study areas have soap berry measurements
all.data <- read.LTEM.data(study.type="Alpine")
...
# select SHEPCAN species
#all.data$user.data <- all.data$user.data[ all.data$user.data$SPECIES_CODE == "SHEPCAN",]
xtabs(~SPECIES_CODE, data=all.data$user.data, exclude=NULL, na.action=na.pass)
...
projects <- projects[ projects$SPI_PROJECT_ID %in% PROJECT_CODE.Alpine,]
...
plyr::l_ply(sort(unique(projects$"STUDY_AREA_NAME"))[1:3], function(x){
  cat("\n\n*******************************************************************************\n")
  cat("*******************************************************************************\n")
  cat("*******************************************************************************\n")
  ...
  quarto::quarto_render("alpine-single-site.qmd",
