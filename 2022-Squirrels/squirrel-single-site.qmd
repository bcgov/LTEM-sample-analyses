...
data.extract <- read.LTEM.data(study.type="Squirrels",
                                         site.names=params$Study.area.name, 
                                         sheets=c("General Survey","Transect Information"))
...
transect.df <- data.extract$extracted.sheets$"Transect Information"
squirrel.df <- data.extract$extracted.sheets$"General Survey"
...
squirrel.df$Date.new<- as.Date(squirrel.df$Date)

select <- is.na(squirrel.df$Date.new)
if(any(select)){
...
squirrel.df$Date <- squirrel.df$Date.new

squirrel.df$Year <- lubridate::year(squirrel.df$Date)
...
# notice that for squirrels, the year is not stored in the Transect Information sheet and
# needs to be extracted from the workbook name
year.start <- 4+regexpr("dct_",transect.df$workbook, fixed=TRUE)
transect.df$Year <- as.numeric(substr(transect.df$workbook,year.start, year.start+3)) 


cat("\n\nThe number of records by year in the Transect Information sheets are \n")
...
<!-- ## Call type - NOW APPEARS TO BE ONLY AUDIBLE-->
...
with multiple plots measured on each transect that are also repeatedly 
measured over time. 
...
prelim.plot <- ggplot(data=count.transect, aes(x=Year, y=log(n.calls+.5), color=Transect.label, shape=Transect.label))+
...
$$log(AvgCalls+.5) \sim Year + TransectF(R) + YearF(R)$$
where 
...
count.fit <- lmerTest::lmer(log(n.calls+.5) ~ Year + (1|Transect.labelF) + (1|YearF), data=count.transect)
